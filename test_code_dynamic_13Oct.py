...
def generate_custom_pdf(data=None, filename="test_output.pdf"):
    ...
    # ---------------- Header Components ----------------
    def build_header():
        """Returns list of flowables representing the standard header."""
        header_flow = []
        ...
        # --- Date and Plan Title ---
        date_str = nd.get('FMO-MATURE-DATE', '')
        contract_date_fw = nd.get('CYCLE-DATE', '')
        plan_name_fw = fixed_width(nd.get('PLAN-MARKET-NAME', ''), FIELD_SPECS['PLAN_MARKET_NAME'])
        ...
        top_table = Table([["", header_block, main_info_block]], colWidths=[30, 240, 270])
        top_table.setStyle(TableStyle([
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ("LEFTPADDING", (0, 0), (-1, -1), 0),
            ("RIGHTPADDING", (0, 0), (-1, -1), 0)
        ]))
        header_flow.append(top_table)
        header_flow.append(Spacer(1, 8))

        # --- Assistance Message ---
        assist_table = Table([
            ["", Paragraph(assistance_msg, styles["Body"])]
        ], colWidths=[270, 270])
        assist_table.setStyle(TableStyle([
            ("LEFTPADDING", (0, 0), (-1, -1), 0),
            ("RIGHTPADDING", (0, 0), (-1, -1), 0),
        ]))
        header_flow.append(assist_table)
        header_flow.append(Spacer(1, 8))
        ...
        header_flow.append(FMOBar(LETTER[0] - 30, 22, "F M O   M A T U R I T Y   N O T I C E"))
        header_flow.append(Spacer(1, 12))
        return header_flow

    # ---- Page 1 ----
    flow.extend(build_header())

    # ---- Page 1 body ----
    for line in notice_info.values():
        if line:
            flow.append(Paragraph(line[0], styles["Body"]))
        else:
            flow.append(Spacer(1, 4))
    ...
    # ---- Add your page 2 body content here ----
    flow.append(Paragraph("This is the second page body content area.", styles["Body"]))

    doc.build(flow)
    print(f"✅ PDF generated with extra page: {filename}")
